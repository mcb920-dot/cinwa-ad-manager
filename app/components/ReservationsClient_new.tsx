...
export default function ReservationsClient({ reservations: initial, months, categories }: Props) {
  ...
  async function handleStatusChange(id: number, status: Status) {
    setProcessing(id)
    clearError(id)
    const result = await supabase.from('reservations').update({ status }).eq('id', id).select()
    console.log('[cinwa] update result:', JSON.stringify(result))
    const { data, error } = result
    if (error) {
      setError(id, `DB error: ${error.message}`)
    } else if (!data || data.length === 0) {
      setError(id, 'No rows updated — RLS is blocking writes. Run the SQL policies in Supabase.')
    } else {
  ...
  async function handleMarkPaid(r: Reservation) {
    ...
    if (r.package_type === 'featured_partner') {
      const { data, error } = await supabase.from('partner_spots').insert([{
        month_id:     r.month_id,
      ...
      console.log('[cinwa] partner_spots insert:', JSON.stringify({ data, error }))
      if (error) {
        createErr = error.code === '23505'
          ? 'Category already taken for this month. Remove the duplicate in Partner Spots first.'
          : `Failed to create partner spot: ${error.message}`
      } else if (!data || data.length === 0) {
        createErr = 'Insert returned no data — RLS may be blocking inserts on partner_spots.'
      }
    } else if (r.package_type === 'cover_sponsor') {
      const { data, error } = await supabase.from('cover_sponsors').insert([{
        month_id:     r.month_id,
        company_name: r.company_name,
        position:     '',
        paid:         true,
      ...
      console.log('[cinwa] cover_sponsors insert:', JSON.stringify({ data, error }))
      if (error) {
        createErr = `Failed to create cover sponsor: ${error.message}`
      } else if (!data || data.length === 0) {
        createErr = 'Insert returned no data — RLS may be blocking inserts on cover_sponsors.'
      }
    }
    ...
    const result = await supabase.from('reservations').update({ status: 'Paid' }).eq('id', r.id).select()
    console.log('[cinwa] mark paid update:', JSON.stringify(result))
    const { data: statusData, error: statusErr } = result
    if (statusErr) {
      setError(r.id, `Record created but status update failed: ${statusErr.message}`)
    } else if (!statusData || statusData.length === 0) {
      setError(r.id, 'Record created but status update returned no rows — check RLS UPDATE policy.')
    } else {
